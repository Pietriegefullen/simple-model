...
def HeteroMicrobe(Biomass, Sub1, Sub2, ATPprod, Yatp, Km1, Vmax): 
    MM1 = Sub1 / (Km1 + Sub1) if Sub1 > 0 else 0
    deltaSub1 = MM1 * Vmax  * Biomass #*100
    ATP = deltaSub1 * ATPprod 
    deltaBiomass1 = ATP * Yatp # growth rate = deltaBiomass1 = ATPprod*Yatp*Substrate, *100 um g zu mg
    
    MM2 = Sub2 / (5*Km1 + Sub2) if Sub2 > 0 else 0
    deltaSub2 = MM2 * Vmax * Biomass *100 # die 5 ist erfunden!!!! 
...
def AutoMicrobe(Biomass, Sub1, Sub2, ATPprod, Yatp, Km1, Km2, Vprod_max, Stoch): 
    #deltaSub1 = Sub1 / (Km1 + Sub1) * Vprod_max  * Biomass * 100 # Gram pro mikromol
    MM_1 = Sub1 / (Km1 + Sub1) if Sub1 > 0 else 0
    MM_2 = Sub2 / (Km2 + Sub2) if Sub2 > 0 else 0
    deltaSub1 = MM_1 * MM_2 * Vprod_max  * Biomass #* 100 # Gram pro mikromol
...
def Fermenters(Biomass, Sub1, Sub2 , Vmax_Ferm, ATPprod_Ferm,Yatp_Ferm):
    ...
    Yatp = Yatp_Ferm # 10e-3 # Fenchel  g pro mikromol ATP 
...
def AltE(Biomass, Acetate, AltEpool, Stoch_ALtE, Vprod_max_AltE,ATPprod_AltE,Yatp_AltE):
    ...
    Yatp = Yatp_AltE  # 10e-3 # Fenchel g pro mikromol ATP 
...
def Homo(Biomass, CO2, H2,ATPprod_Homo,Yatp_Homo):
    ...
    Yatp = Yatp_Homo  # 10e-3 #Fenchel
    Km1 = 0.05 / SOIL_DENSITY # 0.05 from Song
    Km2 = 0.01 / SOIL_DENSITY # 0.01 from Song
    Vprod_max = 0.15 / SOIL_DENSITY # 0.15 from Song, Laut Ye 3 bis 6 mal schneller als Hydro
