...
def Gibbsreaction(Gst, T , reactants, products):
    
    prod_products = 1
    for (stoichiometry, concentration) in products:
        concentration = concentration/10**6
        prod_products = prod_products*(concentration**stoichiometry)
    
    prod_reactants = 1
    for (stoichiometry, concentration) in reactants:
        concentration = concentration/10**6
        prod_reactants = prod_reactants*(concentration**stoichiometry)
    
    Q = prod_products/prod_reactants
    
    R = 8.3144626 # constant R in J / (K mol)
    R = R/1000 # in kJ/(K mol)
    Gr = Gst + R * T * np.log(Q)
...
def HeteroMicrobe(Biomass, Sub1, Sub2, w, Km1, Vmax, Sensenmann, Kmb, Kmh, Gr):  #ATPprod, Yatp, 
...
def AutoMicrobe(Biomass, Sub1, Sub2, w, Km1, Km2, Vprod_max, Stoch, Sensenmann, Kmb, Gr): # ATPprod, Yatp, 
    MM_1 = Sub1 / (Km1 + Sub1) if Sub1 > 0 else 0
    MM_2 = Sub2 / (Km2 + Sub2) if Sub2 > 0 else 0
    MMB = Biomass / (Kmb + Biomass) if Biomass >0 else 0

    deltaSub1Resp = MM_1 * MM_2  * Vprod_max * Biomass#* MMB # * (1- np.exp(min(0, Gr))) micromol
...
def Fermenters(Biomass, Sub1, Sub2 , Vmax, w_Ferm, Sensenmann, Kmb, Kmh, Gr):
    Km1 = 10 / SOIL_DENSITY    # 10 from Song  mikromol pro gram 
    #Vmax = 0.5e6 / SOIL_DENSITY # 0.5 from Song
        
    deltaBiomass, deltaSub1, deltaSub2, ToteMicroben = HeteroMicrobe(Biomass, Sub1, Sub2, w_Ferm, Km1, Vmax, Sensenmann, Kmb, Kmh, Gr)
    
    return deltaBiomass, deltaSub1, deltaSub2, ToteMicroben



def FE(Biomass, Acetate, FEpool, Stoch_FE, Vprod_max,w_FE,Sensenmann, Kmb, Gr):
    ...
    Km2 = 0                     # damit FE Pool keine michaelis menten gleichung hat 
    #Vprod_max = 0.3* 10**6/ SOIL_DENSITY    # geschätzt
    Stoch = Stoch_FE #jason 2001: ca 7 , weil acetate + 8Fe(III)  + 4H2O ---> 2HCO3 + 8Fe(II) + 9H+

    deltaBiomass, deltaAcetate, deltaFE, ToteMicroben = AutoMicrobe(Biomass,  Acetate, FEpool, w_FE, Km1, Km2, Vprod_max, Stoch, Sensenmann, Kmb,Gr)

    return deltaBiomass, deltaAcetate, deltaFE, ToteMicroben



def Hydrotrophes(Biomass, CO2, H2, w_Hydro, Vprod_max,Sensenmann, Kmb, Gr ):
    ...
    deltaBiomass, deltaCO2, deltaH2, ToteMicroben = AutoMicrobe(Biomass, CO2, H2, w_Hydro, Km1, Km2, Vprod_max, Stoch, Sensenmann, Kmb, Gr)
    
    return deltaBiomass, deltaCO2, deltaH2, ToteMicroben


def Homo(Biomass, CO2, H2, w_Homo, Vprod_max, Sensenmann, Gr):
    #Sensenmann = 0
    ...
    deltaBiomass, deltaCO2, deltaH2, ToteMicroben = AutoMicrobe(Biomass, CO2, H2, w_Homo, Km1, Km2, Vprod_max, Stoch,Sensenmann, 0, Gr)
...
def Acetoclast(Biomass, Acetate, w_Ace, Vprod_max, Sensenmann, Kmb, Gr):
    Km1 = 0.05 / SOIL_DENSITY #0.05 / SOIL_DENSITY   # 0.05 from song, Wert sollte 10 mal höher sein als bei FE laut roden2003 bei 12Mikromol !!!!
    Km2 = 0 / SOIL_DENSITY      # 
    #Vprod_max = 0.5/ SOIL_DENSITY # 0.5 from song
    Stoch = 0
    

    deltaBiomass, deltaAcetate, _, ToteMicroben = AutoMicrobe(Biomass, Acetate, 1, w_Ace, Km1, Km2, Vprod_max, Stoch,Sensenmann, Kmb, Gr)
