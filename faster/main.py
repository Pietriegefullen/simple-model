...
def save_model(specimen_index, site, model_parameters, prefix = '', save_dir = None):
    ...
    parameter_file = os.path.join(save_dir, file_name)
...
def fit_model(specimen_index, site, pathway_names = None):
    ...
    # some model parameters are fixed and will not be optimized
...
def mean_model_and_datapoints():
    """
    plots CO2 and CH4 for one model and 
    overlays boxplots of measured data
    ...
    # load a model for comparison with measured data
    ...
    # extract all days with measurements from all specimens for the site
    ...
    # filter for days where more than a certain number of specimens were measured
    ...
    # add measured CO2 boxplots to CO2 plot from model
    plt.figure('CO2')    
    plt.boxplot(list(CO2_boxplot_data.values()), 
                showmeans=True,
                positions = list(CO2_boxplot_data.keys()))
    
    # add measured CH4 boxplots to CH4 plot from model
    plt.figure('CH4')    
