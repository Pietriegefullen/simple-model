...
def optifun(xdata,f_CH4, f_CO2, f_alte, w_CH4, w_CO2, w_alte, w_CH4_heil):
    #print(xdata)
    xtage = xdata[0:int(len(xdata)/2)]
    #print(type(xtage))
    CH4, CO2, _,_,_,_,_,_,_ = simplefun(xtage,f_CH4, f_CO2, f_alte, w_CH4 , w_CO2, w_alte, w_CH4_heil)
    return CH4 + CO2 # setzt die für uns interessanten Ausgaben in curvefit format zusammen
    
    


def simplefun(xtage, f_CH4, f_CO2, f_alte, w_CH4, w_CO2, w_alte, w_CH4_heil): # xdata muss eingegeben werden für curve_fit in multifit
    n = max(xtage)
    ...
    for t in range(1,n+1): # iteriert über 100 Zeitschritte
        ...
        delta = Cdec(Cpool[-1], AltEpool[-1],Microben_CH4[-1],Microben_CO2[-1], Microben_AltE[-1], CH4[-1], CO2[-1], AceCO2[-1], Acetate[-1], Microben_CH4_krank[-1], f_CH4, f_CO2, f_alte, w_CH4, w_CO2, w_alte, w_CH4_heil)
